...
public class GetChallengeServlet extends HttpServlet{
    ...
    public void  service(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException 
    {
		...
		PrintWriter out = null;
		    ...
		    String hash = request.getHeader("messageDigest");//No I18N
		    try{
			    CryptoGraphAPI crypto = NmsUtil.getCryptoGraphAPI();
			    if (crypto != null)
			    {   
				    hash = java.net.URLDecoder.decode(hash);
				    if(hash == null || hash.trim().equals("")){//No I18N
					    out = response.getWriter();
					    out.println("No such user");//No I18N
				    }   
				    hash = crypto.deCrypt(hash);
			    }   
			    /*else
			      {   
			      hash = AuthUtil.decryptString(hash,"WqrTnvA");//No I18N
			      }*/ 
		    } catch (Exception ex) {
